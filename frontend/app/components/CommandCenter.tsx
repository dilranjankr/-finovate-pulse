...
export default function CommandCenter({
  ...
  return (
    <div className="page">
      ...
      <div className="row3a">
        <div className="panel">
          <div className="ph"><h3>Hours Trend <span className="hl">billable vs non-billable · 7-day forecast</span></h3></div>
          <TrendLines data={trendData} />
        </div>
        <div className="panel">
          <div className="ph"><h3>Hours Distribution</h3></div>
          <div className="donut-wrap" style={{ marginTop: 8 }}>
            <div style={{ width: 160 }}><Donut data={dist} colors={["#0f9043", "#cdd4e0"]} height={210} center={{ value: n0(distTotal) + "h", label: "Total Hours" }} /></div>
            <div className="legend">
              <div className="li"><span className="dot" style={{ background: "#0f9043" }} /><span className="nm">Billable</span><span className="vl">{n0(dist[0].value)}h</span><span className="pc">{distTotal ? ((dist[0].value / distTotal) * 100).toFixed(1) : 0}%</span></div>
              <div className="li"><span className="dot" style={{ background: "#cdd4e0" }} /><span className="nm">Non-Billable</span><span className="vl">{n0(dist[1].value)}h</span><span className="pc">{distTotal ? ((dist[1].value / distTotal) * 100).toFixed(1) : 0}%</span></div>
            </div>
          </div>
        </div>
        <div className="panel">
          <div className="ph"><h3>Grade Distribution</h3></div>
          <GradeBars data={data.grade_distribution} height={240} />
        </div>
      </div>
      ...
      <div className="row-perf3">
        <div className="panel">
          <div className="ph">
            <div className="ptabs">
              <button className={perfTab === "top" ? "on" : ""} onClick={() => setPerfTab("top")}>Top Performers</button>
              <button className={perfTab === "bottom" ? "on" : ""} onClick={() => setPerfTab("bottom")}>Bottom Performers</button>
            </div>
            <span className="viewall" onClick={() => setModal("employees")}>View all →</span>
          </div>
          {(perfTab === "top" ? data.top3 : data.bottom3).map((e, i) => (
            <div className="prow clk" key={e.name} onClick={() => openEmployee(e.name)}><span className="rank">{i + 1}</span><span className="avatar" style={{ background: avatarColor(e.name) }}>{initials(e.name)}</span>
              <div className="pinfo"><div className="nm">{e.name}</div><div className="tm">{e.team}</div></div>
              {perfTab === "top"
                ? <span className="num" style={{ fontSize: 11, color: "var(--green)", fontWeight: 700 }}>{n0(e.billable)}h</span>
                : <span className="num" style={{ fontSize: 11, color: "var(--red)", fontWeight: 700 }}>{n0(e.utilization)}%</span>}
              <span className={`grade ${gradeCls(e.grade)}`}>{e.grade}</span></div>
          ))}
        </div>
        <div className="panel">
          <div className="ph"><h3>Task Summary</h3><span className="hl" style={{ marginLeft: 0 }}>{n0(taskTotal)}</span></div>
          <div className="donut-wrap">
            <div style={{ width: 116 }}><Donut data={data.task_summary} colors={TASKCOL} height={138} center={{ value: n0(taskTotal), label: "Tasks" }} /></div>
            <div className="legend">{data.task_summary.map((t, i) => (<div className="li" key={t.name}><span className="dot" style={{ background: TASKCOL[i] }} /><span className="nm">{t.name}</span><span className="vl">{n0(t.value)}</span><span className="pc">{taskTotal ? Math.round((t.value / taskTotal) * 100) : 0}%</span></div>))}</div>
          </div>
        </div>
        <div className="panel">
          <div className="ph"><h3>Alerts &amp; Exceptions</h3></div>
          {data.alerts.map((a, i) => { const Ico = alertIcons[i % alertIcons.length]; const col = alertColors[i % alertColors.length];
            return <div className="alert" key={a.title}><span className="ico" style={{ background: col + "1c", color: col }}><Ico /></span><span className="t">{a.title}</span><span className="cnt" style={{ background: col }}>{a.count}</span></div>; })}
        </div>
      </div>

      {/* HEALTH ROW */}
      <div className="row-quad health">
        <div className="panel">
          <div className="ph"><h3>Client Health</h3></div>
          <div className="donut-wrap">
            <div style={{ width: 118 }}><Donut data={[{ name: "Active", value: ch.active }, { name: "At Risk", value: ch.at_risk }, { name: "Inactive", value: ch.inactive }]} colors={HEALTHCOL} height={138} center={{ value: String(chTotal), label: "Clients" }} /></div>
            <div className="legend">
              <div className="li"><span className="dot" style={{ background: HEALTHCOL[0] }} /><span className="nm">Active</span><span className="vl">{ch.active}</span><span className="pc">{chTotal ? Math.round((ch.active / chTotal) * 100) : 0}%</span></div>
              <div className="li"><span className="dot" style={{ background: HEALTHCOL[1] }} /><span className="nm">At Risk</span><span className="vl">{ch.at_risk}</span><span className="pc">{chTotal ? Math.round((ch.at_risk / chTotal) * 100) : 0}%</span></div>
              <div className="li"><span className="dot" style={{ background: HEALTHCOL[2] }} /><span className="nm">Inactive</span><span className="vl">{ch.inactive}</span><span className="pc">{chTotal ? Math.round((ch.inactive / chTotal) * 100) : 0}%</span></div>
            </div>
          </div>
        </div>
        <div className="panel">
          <div className="ph"><h3>Project Health <span className="hl">by team utilization</span></h3></div>
          <div className="donut-wrap">
            <div style={{ width: 118 }}><Donut data={[{ name: "On Track", value: ph.on_track }, { name: "At Risk", value: ph.at_risk }, { name: "Delayed", value: ph.delayed }]} colors={HEALTHCOL} height={138} center={{ value: String(phTotal), label: "Teams" }} /></div>
            <div className="legend">
              <div className="li"><span className="dot" style={{ background: HEALTHCOL[0] }} /><span className="nm">On Track</span><span className="vl">{ph.on_track}</span><span className="pc">{phTotal ? Math.round((ph.on_track / phTotal) * 100) : 0}%</span></div>
              <div className="li"><span className="dot" style={{ background: HEALTHCOL[1] }} /><span className="nm">At Risk</span><span className="vl">{ph.at_risk}</span><span className="pc">{phTotal ? Math.round((ph.at_risk / phTotal) * 100) : 0}%</span></div>
              <div className="li"><span className="dot" style={{ background: HEALTHCOL[2] }} /><span className="nm">Delayed</span><span className="vl">{ph.delayed}</span><span className="pc">{phTotal ? Math.round((ph.delayed / phTotal) * 100) : 0}%</span></div>
            </div>
          </div>
        </div>
        <div className="panel">
          <div className="ph"><h3>Team Utilization <span className="hl">top teams</span></h3></div>
          <TeamGauges data={teamGauge} height={210} />
        </div>
        <div className="panel">
          <div className="ph"><h3>Resource Overview</h3></div>
          <div className="resbar" style={{ marginTop: 12 }}>
            <div className="r"><span className="k">Capacity Used</span><b>{n0(res.capacity)}%</b></div>
            <div className="track"><span style={{ width: `${res.capacity}%`, background: "#0f9043" }} /></div>
            <div className="r"><span className="k">Availability</span><b>{n0(res.availability)}%</b></div>
            <div className="track"><span style={{ width: `${res.availability}%`, background: "#2f6fbf" }} /></div>
          </div>
          {data.insights.slice(0, 2).map((s, i) => <div className="insight" key={i}><span className="ic"><CheckCircle2 /></span><span>{s}</span></div>)}
        </div>
      </div>
